...
std::vector<size_t> SudokuSolver::_GetAvailableNumbersForCell(size_t i,
                                                              size_t j) const {
  ...
  for (size_t ii = si * _sudoku.SubSize(); ii < (si + 1) * _sudoku.SubSize();
       ++ii) {
    for (size_t jj = sj * _sudoku.SubSize(); jj < (sj + 1) * _sudoku.SubSize();
         ++jj) {
