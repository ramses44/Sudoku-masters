...
Sudoku::Sudoku(const std::vector<std::vector<Cell>> &matrix)
    : _size(matrix.size()) {
  if (!IsValidSize(_size)) {
    throw std::invalid_argument("Invalid size!");
  }
...
Sudoku::Sudoku(const Sudoku &rhs)
    : _size(rhs._size), _field(_size, std::vector<Cell>(_size)) {
  for (size_t i = 0; i < _size; ++i) {
    for (size_t j = 0; j < _size; ++j) {
...
std::vector<Cell> &Sudoku::operator[](size_t ind) { return _field[ind]; }

const std::vector<Cell> &Sudoku::operator[](size_t ind) const {
  return _field.at(ind);
}

size_t Sudoku::size() const { return _size; }

size_t Sudoku::SubSize() const { return sqrt(_size); }

size_t Sudoku::CellsCount() const { return _size * _size; }
      ...
      for (int ii = 0; ii < sub_size; ++ii) {
        for (int jj = 0; jj < sub_size; ++jj) {
          if (_field[i * sub_size + ii][j * sub_size + jj].number != 0 &&
              ++counter[_field[i * sub_size + ii][j * sub_size + jj].number] >
                  1) {
            return false;
          }
        }
...
bool Sudoku::SetNumber(int number, int row, int col) {
  ...
  for (int i = 0; i < sub_size; ++i) {
    for (int j = 0; j < sub_size; ++j) {
      _field[sec_row * sub_size + i][sec_col * sub_size + j].noted.erase(
          number);
    }
  }
