...
void SudokuBuilder::GenerateField() {
  ...
  for (size_t i = 0; i < shuffleStepsInGenerating; ++i) {
    ...
    case 2: {
      ...
      SwapLines(rand() % 2 ? Dimension::horizontal : Dimension::vertical,
                section * _sudoku.SubSize() + rand() % _sudoku.SubSize(),
                section * _sudoku.SubSize() + rand() % _sudoku.SubSize());
      break;
    }
    case 3:
      SwapSections(rand() % 2 ? Dimension::horizontal : Dimension::vertical,
                   rand() % _sudoku.SubSize(), rand() % _sudoku.SubSize());
      break;
    case 4:
      SwapNumbers(rand() % _sudoku.size() + 1, rand() % _sudoku.size() + 1);
    }
  }
}
...
bool SudokuBuilder::TryThrowCell(Difficulty difficulty) {
  ...
  auto forward_solved = std::async(
      [&]() { return SudokuSolver(_sudoku, false).Solve(difficulty); });
  auto reverse_solved = std::async(
      [&]() { return SudokuSolver(_sudoku, true).Solve(difficulty); });
...
void SudokuBuilder::SwapLines(SudokuBuilder::Dimension dim, size_t first,
                              size_t second) {
  for (size_t i = 0; i < _sudoku.size(); ++i) {
    if (dim == Dimension::vertical) {
...
void SudokuBuilder::SwapSections(SudokuBuilder::Dimension dim, size_t first,
                                 size_t second) {
  for (size_t i = 0; i < _sudoku.SubSize(); ++i) {
    SwapLines(dim, first * _sudoku.SubSize() + i,
              second * _sudoku.SubSize() + i);
  }
}
