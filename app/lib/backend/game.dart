...
const String SERVER_ADDR = 'http://192.168.31.23:12122';
...
class SudokuGame {
  ...
  SudokuGame.generate(Difficulty difficulty, int size,
    ...
    var response = http.SyncHttpClient.getUrl(Uri.parse('$SERVER_ADDR/generate?size=$size&difficulty=${difficultyToString(difficulty)}')).close();

    if (response.statusCode != 200) {
      sudoku = ValueNotifier(SudokuBuilder(size).build(difficulty));
    } else {
      var data = response.body!.split("\n");
      var unsolved = data.sublist(0, size + 1).map((e) => e.split(" ")).toList();
      var solved = data.sublist(size + 1).map((e) => e.split(" ")).toList();

      List<List<Cell>> field = List.generate(
          size, (i) => List.generate(size, (j) =>
          Cell(int.parse(unsolved[i][j]), int.parse(solved[i][j]))));

      sudoku = ValueNotifier(Sudoku.fromMatrix(field));
    }
